...
int Vmc_Open(iop_file_t *f, const char *name, int flags, int mode)
{
    ...
    DEBUGPRINT(1, "vmc_fs: Open %i %s", f->unit, name);
    ...
    if (dirent_cluster == ROOT_CLUSTER) {
        ...
        if (flags & O_CREAT) {
            ...
            DEBUGPRINT(2, "Creating file %s in parent folder %s", filename, path);
            ...
            if (dirent_cluster == ERROR_CLUSTER) {
                ...
                DEBUGPRINT(2, "vmc_fs: open failed on %s", name);
    ...
    } else if ((!(dirent.mode & DF_EXISTS)) || (flags & O_TRUNC))  // File found but is hidden, or trunc the file
    {
        ...
        while (1) {
            ...
            if (current_cluster == FREE_CLUSTER) {
                ...
                DEBUGPRINT(8, "Last cluster of file at %u", last_cluster);
                ...
                DEBUGPRINT(8, "Testing cluster %u ... value is %u", last_cluster, current_cluster);
    ...
    DEBUGPRINT(2, "vmc_fs: File %s opened with length %u", name, fprivdata->file_length);
...
int Vmc_Read(iop_file_t *f, void *buffer, int size)
{
    ...
    DEBUGPRINT(1, "Read %d bytes", size);
    ...
    DEBUGPRINT(3, "Reading in %i bytes", size);

    //  we need to read in size, unless size is larger then length
    if (size > fprivdata->file_length - fprivdata->file_position) {

        DEBUGPRINT(3, "Adjusting size to read in to be %u", fprivdata->file_length - fprivdata->file_position);
    ...
    while (data_read < size) {
        ...
        DEBUGPRINT(3, "There are %i bytes remaining", size - data_read);
        DEBUGPRINT(3, "There are %i bytes left in cluster %u", (fprivdata->cluster_offset == 0) ? 0 : g_Vmc_Image[f->unit].cluster_size - fprivdata->cluster_offset, fprivdata->file_cluster + fprivdata->gendata.first_allocatable);
        ...
        if (size - data_read < g_Vmc_Image[f->unit].cluster_size - fprivdata->cluster_offset) {
            ...
            DEBUGPRINT(3, "Read in %i bytes", size);
        ...
        } else  //  We can copy in the rest of the cluster, since there is space in the buffer passed to us
        {
            ...
            DEBUGPRINT(3, "Read in %i bytes", g_Vmc_Image[f->unit].cluster_size - fprivdata->cluster_offset);
...
int Vmc_Write(iop_file_t *f, void *buffer, int size)
{
    ...
    DEBUGPRINT(1, "Write %i bytes", size);
    ...
    DEBUGPRINT(5, "vmc_fs: File lenght in cluster before this write: %d", num_clusters);
    ...
    DEBUGPRINT(5, "vmc_fs: File lenght in cluster after this write : %d", num_clusters_required);

    if (num_clusters_required > num_clusters) {

        fprivdata->file_length = fprivdata->file_position + size;

        DEBUGPRINT(3, "vmc_fs: File position:  %u", fprivdata->file_position);
        DEBUGPRINT(3, "vmc_fs: Size to write:  %d", size);
        DEBUGPRINT(3, "vmc_fs: File length  :  %u", fprivdata->file_length);

        //  now determine how many clusters we need forthis write
        unsigned int clusters_needed = num_clusters_required - num_clusters;

        DEBUGPRINT(3, "We need to allocate %u more clusters for storage", clusters_needed);
        ...
        if (fprivdata->file_startcluster == EOF_CLUSTER) {
            ...
            DEBUGPRINT(3, "Initial cluster at %u", free_cluster);
            ...
            while (1) {
                ...
                if (current_cluster == FREE_CLUSTER) {
                    ...
                    DEBUGPRINT(10, "vmc_fs: Testing cluster %u ... value is FREE_CLUSTER", last_cluster);

                } else if (current_cluster == EOF_CLUSTER) {

                    DEBUGPRINT(10, "Testing cluster %d ... value is EOF_CLUSTER", i);
                    break;

                } else {

                    DEBUGPRINT(10, "vmc_fs: Testing cluster %u ... value is %u", last_cluster, current_cluster);
                }
            ...
            DEBUGPRINT(3, "Last cluster of file at %u", last_cluster);
            ...
            DEBUGPRINT(3, "Restart cluster for the file at %u", free_cluster);
        ...
        for (i = 0; i < clusters_needed; i++) {
            ...
            DEBUGPRINT(3, "Adding cluster %u to the chain for the file", free_cluster);
    ...
    while (data_written < size) {
        ...
        if ((size - data_written) >= (g_Vmc_Image[f->unit].cluster_size - fprivdata->cluster_offset)) {
            ...
            if (fprivdata->cluster_offset == 0) {
                ...
                DEBUGPRINT(4, "Writing to cluster %u", current_cluster);
                ...
                DEBUGPRINT(4, "Writing to cluster %u at offset %u for length %u", current_cluster, fprivdata->cluster_offset, g_Vmc_Image[f->unit].cluster_size - fprivdata->cluster_offset);
            ...
            DEBUGPRINT(4, "Writing to cluster %u at offset %u for length %u", current_cluster, fprivdata->cluster_offset, size - data_written);
        ...
        DEBUGPRINT(5, "Wrote %i bytes", sizewritten);
...
int Vmc_Ioctl(iop_file_t *f, int cmd, void *data)
{
    ...
    switch (cmd) {
        ...
        default:
            ...
            DEBUGPRINT(1, "Unrecognized ioctl command %d", cmd);
            break;
    }
...
int Vmc_Remove(iop_file_t *f, const char *path)
{
    ...
    if (dirent_cluster == ROOT_CLUSTER) {
        ...
        DEBUGPRINT(2, "remove failed. %s not found.", path);
    ...
    if (dirent.mode & DF_FILE) {
        ...
        DEBUGPRINT(2, "file %s removed.", path);

    } else {

        DEBUGPRINT(2, "remove failed. %s is not a valid file.", path);
...
int Vmc_Mkdir(iop_file_t *f, const char *path1, int mode)
{
    ...
    if (dirent_cluster == ROOT_CLUSTER) {
        ...
        DEBUGPRINT(2, "Creating folder %s in parent folder %s", newdir, (path[0] == '\0') ? "Root" : path);
        ...
        DEBUGPRINT(3, "parent_cluster  = %u", parent_cluster);
        DEBUGPRINT(3, "dir_cluster    = %u", parent_dirent.cluster);
        DEBUGPRINT(3, "dirent.name    = %s", parent_dirent.name);
        DEBUGPRINT(3, "dirent.length  = %u", parent_dirent.length);
        DEBUGPRINT(3, "dirent.mode    = %X", parent_dirent.mode);
        DEBUGPRINT(3, "dirent_page    = %i", parent_gendata.dirent_page);
        ...
        if (new_dirent_cluster == ERROR_CLUSTER) {
            ...
            DEBUGPRINT(2, "mkdir failed on %s", path);
    ...
    } else  // directory allready exist, so test DF_EXISTS flag
    {
        ...
        if (dirent.mode & DF_EXISTS) {
            ...
            DEBUGPRINT(2, "mkdir failed on %s. Directory allready exist.", path1);
            ...
            DEBUGPRINT(8, "mkdir directory %s allready exist but is hidden. Changing attributs.", path1);

            DEBUGPRINT(8, "Following fat table cluster %u", dirent.cluster);

            unsigned int pseudo_entry_cluster = getFatEntry(gendata.fd, dirent.cluster, gendata.indir_fat_clusters, FAT_VALUE);

            DEBUGPRINT(8, "Changing cluster mask of fat table cluster %u.", pseudo_entry_cluster);
            ...
            DEBUGPRINT(8, "Changing direntry %s attributs.", path1);
            ...
            DEBUGPRINT(8, "Restoring EOF cluster at %u.", pseudo_entry_cluster);

            setFatEntry(gendata.fd, pseudo_entry_cluster, EOF_CLUSTER, gendata.indir_fat_clusters, FAT_SET);

            struct direntry pseudo_entries;

            DEBUGPRINT(8, "Updating pseudo entries time stamps at cluster %u / page %u.", dirent.cluster + gendata.first_allocatable, (dirent.cluster + gendata.first_allocatable) * g_Vmc_Image[f->unit].header.pages_per_cluster);
            ...
            DEBUGPRINT(8, "Updating pseudo entries time stamps at cluster %u / page %u.", dirent.cluster + gendata.first_allocatable, (dirent.cluster + gendata.first_allocatable) * g_Vmc_Image[f->unit].header.pages_per_cluster + 1);
    ...
    DEBUGPRINT(3, "Directory %s created.", path1);
...
int Vmc_Rmdir(iop_file_t *f, const char *path1)
{
    ...
    if (dirent_cluster == ROOT_CLUSTER) {
        ...
        DEBUGPRINT(2, "rmdir failed. %s not found.", path1);
    ...
    if (!(dirent.mode & DF_EXISTS)) {
        ...
        DEBUGPRINT(2, "rmdir failed. %s is allready removed.", path1);
    ...
    if (dirent.mode & DF_DIRECTORY) {
        ...
        DEBUGPRINT(2, "rmdir failed. %s is not a valid directory.", path1);
    ...
    DEBUGPRINT(3, "Directory %s removed.", path1);
...
int Vmc_Dopen(iop_file_t *f, const char *path)
{
    ...
    DEBUGPRINT(1, "dopen %i %s", f->unit, path);
    ...
    if (dirent_cluster == NOFOUND_CLUSTER) {
        ...
        DEBUGPRINT(2, "dopen failed. %s not found.", path);
    ...
    if (!(dirent.mode & DF_EXISTS)) {
        ...
        DEBUGPRINT(2, "dopen failed. %s is hidden.", path);
    ...
    DEBUGPRINT(2, "Directory %s opened with length %u", path, fprivdata->dir_length);
...
next_entry:
    ...
    DEBUGPRINT(4, "name: %s; cluster %u; length %u; mode 0x%02x", dirent.name, dirent.cluster, dirent.length, dirent.mode);
...
int Vmc_Getstat(iop_file_t *f, const char *path, iox_stat_t *stat)
{
    ...
    DEBUGPRINT(1, "getstat %i %s", f->unit, path);
    ...
    if (dirent_cluster == NOFOUND_CLUSTER) {
        ...
        DEBUGPRINT(2, "getstat failed. %s not found.", path);
...
int Vmc_Chstat(iop_file_t *f, const char *path, iox_stat_t *stat, unsigned int statmask)
{
    ...
    DEBUGPRINT(1, "chstat %i %s", f->unit, path);
    ...
    if (dirent_cluster == NOFOUND_CLUSTER) {
        ...
        DEBUGPRINT(2, "chstat failed. %s not found.", path);
...
int Vmc_Rename(iop_file_t *f, const char *path, const char *new_name)
{
    ...
    if (dirent_cluster == ROOT_CLUSTER) {
        ...
        DEBUGPRINT(2, "rename failed. %s not found.", path);
    ...
    DEBUGPRINT(2, "Object %s renamed to %s", path, new_name);
...
int Vmc_Mount(iop_file_t *f, const char *fsname, const char *devname, int flag, void *arg, int arg_len)
{
    ...
    DEBUGPRINT(2, "Mount %s at mount point: %d", devname, f->unit);
    ...
    if (g_Vmc_Image[f->unit].fd < 0) {
        DEBUGPRINT(1, "Error opening vmc file %s", devname);
        DEBUGPRINT(1, "open error code: %d", g_Vmc_Image[f->unit].fd);
    ...
    if (r != sizeof(struct superblock)) {
        DEBUGPRINT(1, "Error reading vmc file %s", devname);
        DEBUGPRINT(1, "fd: %d, error code:%d", g_Vmc_Image[f->unit].fd, r);
    ...
    if (strcmp((char *)g_Vmc_Image[f->unit].header.magic, ps2_sign)) {
        ...
        DEBUGPRINT(1, "Warning vmc file %s is not formated", devname);
        if (!setDefaultSpec(f->unit)) {
            //  Card size error
            DEBUGPRINT(1, "Error size of vmc file %s is incompatible", devname);
        ...
        DEBUGPRINT(4, "SuperBlock readed from vmc file %s.", devname);

        DEBUGPRINT(4, "SuperBlock Info: magic[40]             : %s", g_Vmc_Image[f->unit].header.magic);
        DEBUGPRINT(4, "SuperBlock Info: page_size             : 0x%02x", g_Vmc_Image[f->unit].header.page_size);
        DEBUGPRINT(4, "SuperBlock Info: pages_per_cluster     : 0x%02x", g_Vmc_Image[f->unit].header.pages_per_cluster);
        DEBUGPRINT(4, "SuperBlock Info: pages_per_block       : 0x%02x", g_Vmc_Image[f->unit].header.pages_per_block);
        DEBUGPRINT(4, "SuperBlock Info: clusters_per_card     : 0x%02x", g_Vmc_Image[f->unit].header.clusters_per_card);
        DEBUGPRINT(4, "SuperBlock Info: first_allocatable     : 0x%02x", g_Vmc_Image[f->unit].header.first_allocatable);
        DEBUGPRINT(4, "SuperBlock Info: last_allocatable      : 0x%02x", g_Vmc_Image[f->unit].header.last_allocatable);
        DEBUGPRINT(4, "SuperBlock Info: root_cluster          : 0x%02x", g_Vmc_Image[f->unit].header.root_cluster);
        DEBUGPRINT(4, "SuperBlock Info: backup_block1         : 0x%02x", g_Vmc_Image[f->unit].header.backup_block1);
        DEBUGPRINT(4, "SuperBlock Info: backup_block2         : 0x%02x", g_Vmc_Image[f->unit].header.backup_block2);
        for (r = 0; g_Vmc_Image[f->unit].header.indir_fat_clusters[r] != 0; r++)
            DEBUGPRINT(4, "SuperBlock Info: indir_fat_clusters[%d] : 0x%02x", r, g_Vmc_Image[f->unit].header.indir_fat_clusters[r]);
        DEBUGPRINT(4, "SuperBlock Info: mc_type               : 0x%02x", g_Vmc_Image[f->unit].header.mc_type);
        DEBUGPRINT(4, "SuperBlock Info: mc_flag               : 0x%02x", g_Vmc_Image[f->unit].header.mc_flag);

        if (g_Vmc_Image[f->unit].header.mc_type != PS2_MEMORYCARD) {
            //  Card is not a PS2 one
            DEBUGPRINT(1, "Error vmc file %s is not a valid PS2 image", devname);
        ...
        DEBUGPRINT(4, "Image file Info: Vmc card type         : %s MemoryCard.", (g_Vmc_Image[f->unit].header.mc_type == PSX_MEMORYCARD ? "PSX" : (g_Vmc_Image[f->unit].header.mc_type == PS2_MEMORYCARD ? "PS2" : "PDA")));
        ...
        DEBUGPRINT(4, "vmc_fs: Image file Info: Number of pages       : %u", g_Vmc_Image[f->unit].total_pages);
        DEBUGPRINT(4, "vmc_fs: Image file Info: Size of a cluster     : %u bytes", g_Vmc_Image[f->unit].cluster_size);
        DEBUGPRINT(4, "vmc_fs: Image file Info: ECC shunk found       : %s", g_Vmc_Image[f->unit].ecc_flag ? "YES" : "NO");
    }
...
int Vmc_Umount(iop_file_t *f, const char *fsname)
{
    ...
    DEBUGPRINT(2, "UnMount %s at mount point: %d", fsname, f->unit);
...
int Vmc_Devctl(iop_file_t *f, const char *path, int cmd, void *arg, unsigned int arglen, void *buf, unsigned int buflen)
{
    ...
    switch (cmd) {
        ...
        default:
            ...
            DEBUGPRINT(1, "Unrecognized devctl command %d", cmd);
            break;
    }
...
int Vmc_Recover(int unit, const char *path1)
{
    ...
    if (dirent_cluster == ROOT_CLUSTER) {
        ...
        DEBUGPRINT(2, "recover failed. %s not found.", path1);
    ...
    DEBUGPRINT(4, "Checking attributs parent directory %s", path);
    ...
    if (parent_cluster == NOFOUND_CLUSTER) {
        ...
        DEBUGPRINT(3, "Unable to recover %s. Parent directory not found.", path1);
    ...
    DEBUGPRINT(6, "parent_cluster  = %u", parent_cluster);
    DEBUGPRINT(6, "dir_cluster    = %u", parent.cluster);
    DEBUGPRINT(6, "dirent.name    = %s", parent.name);
    DEBUGPRINT(6, "dirent.length  = %u", parent.length);
    DEBUGPRINT(6, "dirent.mode    = %X", parent.mode);
    DEBUGPRINT(6, "dirent_page    = %i", parent_gendata.dirent_page);

    if (!(parent.mode & DF_EXISTS)) {

        DEBUGPRINT(3, "Unable to restore %s. Parent directory %s is hidden.", path1, path);
    ...
    if (dirent.mode & DF_DIRECTORY)  // directory case
    {
        ...
        if (dirent.mode & DF_EXISTS) {
            ...
            DEBUGPRINT(2, "recover failed on %s. Directory allready exist.", path1);
            ...
            DEBUGPRINT(8, "recover directory %s allready exist but is hidden. Changing attributs.", path1);
            ...
            DEBUGPRINT(8, "Updating pseudo entries time stamps at cluster %u / page %u.", dirent.cluster + gendata.first_allocatable, (dirent.cluster + gendata.first_allocatable) * g_Vmc_Image[unit].header.pages_per_cluster);
            ...
            DEBUGPRINT(8, "Updating pseudo entries time stamps at cluster %u / page %u.", dirent.cluster + gendata.first_allocatable, (dirent.cluster + gendata.first_allocatable) * g_Vmc_Image[unit].header.pages_per_cluster + 1);
            ...
            DEBUGPRINT(2, "Directory %s recovered.", path1);
    ...
    } else  // file case
    {
        ...
        if (dirent.mode & DF_EXISTS) {
            ...
            DEBUGPRINT(2, "recover failed on %s. File allready exist.", path);
            ...
            DEBUGPRINT(8, "Restoring fat table clusters of file %s", filename);
            ...
            while (1) {
                ...
                if (current_cluster == FREE_CLUSTER) {
                    ...
                    DEBUGPRINT(8, "Restoring End Of File at fat table cluster %u", last_cluster);
                    ...
                    DEBUGPRINT(10, "Testing cluster %u ... value is %u", last_cluster, current_cluster);

                    DEBUGPRINT(8, "Restoring cluster mask at fat table cluster %u", last_cluster);
            ...
            DEBUGPRINT(8, "Restoring direntry at cluster %u / page %u", dirent_cluster + gendata.first_allocatable, (dirent.cluster + gendata.first_allocatable) * g_Vmc_Image[unit].header.pages_per_cluster + gendata.dirent_page);
            ...
            DEBUGPRINT(3, "File %s restored.", path1);
        }
    }
...
unsigned int Vmc_Checkfree(int unit)
{
    ...
    DEBUGPRINT(1, "check free %d", unit);
    ...
    for (i = gendata.first_allocatable; i < gendata.last_allocatable; i++) {
        ...
        if (cluster_value == FREE_CLUSTER) {
            ...
            DEBUGPRINT(10, "Testing cluster %d ... value is FREE_CLUSTER", i);

            DEBUGPRINT(6, "Free cluster found at %d", i);
            ...
            DEBUGPRINT(10, "Testing cluster %d ... value is %u", i, cluster_value);
    ...
    DEBUGPRINT(3, "Total free space: %u", free_space);
    ...
    for (i = gendata.first_allocatable; i < gendata.last_allocatable; i++) {
        ...
        if (cluster_value == FREE_CLUSTER) {
            ...
            if (cluster_mask != MASK_CLUSTER) {
                ...
                DEBUGPRINT(6, "Setting cluster %d as free cluster.", i);
