...
unsigned int getDirentryFromPath(struct direntry *retval, const char *path, struct gen_privdata *gendata, int unit)
{
    ...
    DEBUGPRINT(6, "Searching Direntry corresponding to path: %s", path);
    ...
    for (i = 0; i < length; i++) {
        ...
        DEBUGPRINT(6, "Reading in allocatable cluster %u / page %u", (current_cluster + gendata->first_allocatable), (current_cluster + gendata->first_allocatable) * g_Vmc_Image[unit].header.pages_per_cluster + gendata->dirent_page);
        ...
        DEBUGPRINT(5, "Object Type    : %s", (dirent.mode & DF_DIRECTORY) ? "Folder" : "File");
        DEBUGPRINT(5, "Object Name    : %s", dirent.name);
        DEBUGPRINT(5, "Object Exists  : %s", (dirent.mode & DF_EXISTS) ? "Yes" : "No");
        DEBUGPRINT(5, "Object Length  : %u", dirent.length);
        DEBUGPRINT(5, "Object Cluster : %u", dirent.cluster);
        ...
        if (memcmp(dirent.name, path + pathoffset, strlen(dirent.name)) == 0) {
            ...
            if (path[pathoffset] == '\0' || (path[pathoffset] == '/' && path[pathoffset + 1] == '\0')) {
                ...
                DEBUGPRINT(6, "dir_cluster = %u", dirent.cluster);
                DEBUGPRINT(6, "dirent.name = %s", dirent.name);
                DEBUGPRINT(6, "dirent.length = %u", dirent.length);
...
unsigned int addPseudoEntries(const struct gen_privdata *gendata, const struct direntry *parent, int unit)
{
    ...
    DEBUGPRINT(5, "Adding pseudo entries into fat table cluster %u", pseudo_entries_cluster);
...
unsigned int addObject(struct gen_privdata *gendata, unsigned int parent_cluster, struct direntry *parent, struct direntry *dirent, int unit)
{
    ...
    for (i = 0; i < parent->length; i += g_Vmc_Image[unit].header.pages_per_cluster) {
        ...
        DEBUGPRINT(6, "Following fat table cluster: %u", current_cluster);

        if (getFatEntry(gendata->fd, current_cluster, gendata->indir_fat_clusters, FAT_VALUE) == EOF_CLUSTER) {

            DEBUGPRINT(6, "Last used cluster in fat table %u", current_cluster);
            break;
        }
    ...
    if (parent->length % g_Vmc_Image[unit].header.pages_per_cluster == 0)  // if its even, we need a new cluster for our file
    {
        ...
        DEBUGPRINT(6, "Added new object in fat table cluster %u ( Page %u ) ", current_cluster, current_cluster * g_Vmc_Image[unit].header.pages_per_cluster);
    ...
    } else  // otherwise we can just add the directory entry to the end of the last cluster
    {
        ...
        DEBUGPRINT(5, "Added new object in fat table cluster %u ( Page %u ) ", current_cluster, current_cluster * g_Vmc_Image[unit].header.pages_per_cluster + 1);
...
void removeObject(const struct gen_privdata *gendata, unsigned int dirent_cluster, struct direntry *dirent, int unit)
{
    ...
    while (1) {
        ...
        if (current_cluster == FREE_CLUSTER) {
            ...
            DEBUGPRINT(10, "Testing cluster %u ... value is FREE_CLUSTER", last_cluster);
            ...
            DEBUGPRINT(3, "Last cluster of direntry at %u", last_cluster);
...
unsigned int getFreeCluster(const struct gen_privdata *gendata, int unit)
{
    ...
    for (i = g_Vmc_Image[unit].last_free_cluster; i < gendata->last_allocatable; i++) {
        ...
        if (value == FREE_CLUSTER) {
            ...
            DEBUGPRINT(10, "vmc_fs: Testing fat table cluster %u ... value is FREE_CLUSTER", i - gendata->first_allocatable);

            DEBUGPRINT(6, "vmc_fs: Free cluster found at %u in fat table", i - gendata->first_allocatable);
            ...
            DEBUGPRINT(10, "vmc_fs: Testing fat table cluster %u ... value is EOF_CLUSTER", i - gendata->first_allocatable);

        } else {

            DEBUGPRINT(10, "vmc_fs: Testing fat table cluster %u ... value is %u", i - gendata->first_allocatable, value);
...
int setDefaultSpec(int unit)
{
    ...
    DEBUGPRINT(4, "SuperBlock Info: magic[40]             : %s", g_Vmc_Image[unit].header.magic);
    ...
    DEBUGPRINT(4, "vmc_fs: Image file Info: Number of pages       : %u", g_Vmc_Image[unit].total_pages);
    DEBUGPRINT(4, "vmc_fs: Image file Info: Size of a cluster     : %u bytes", g_Vmc_Image[unit].cluster_size);
    DEBUGPRINT(4, "vmc_fs: Image file Info: ECC shunk found       : %s", g_Vmc_Image[unit].ecc_flag ? "YES" : "NO");
    DEBUGPRINT(3, "vmc_fs: Image file Info: Vmc card type         : %s MemoryCard.", (g_Vmc_Image[unit].header.mc_type == PSX_MEMORYCARD ? "PSX" : (g_Vmc_Image[unit].header.mc_type == PS2_MEMORYCARD ? "PS2" : "PDA")));
    DEBUGPRINT(4, "vmc_fs: SuperBlock Info: page_size             : 0x%02x", g_Vmc_Image[unit].header.page_size);
    DEBUGPRINT(4, "vmc_fs: SuperBlock Info: pages_per_cluster     : 0x%02x", g_Vmc_Image[unit].header.pages_per_cluster);
    DEBUGPRINT(4, "vmc_fs: SuperBlock Info: pages_per_block       : 0x%02x", g_Vmc_Image[unit].header.pages_per_block);
    DEBUGPRINT(4, "vmc_fs: SuperBlock Info: clusters_per_card     : 0x%02x", g_Vmc_Image[unit].header.clusters_per_card);
    ...
    DEBUGPRINT(4, "SuperBlock Info: first_allocatable     : 0x%02x", g_Vmc_Image[unit].header.first_allocatable);
    DEBUGPRINT(4, "SuperBlock Info: last_allocatable      : 0x%02x", g_Vmc_Image[unit].header.last_allocatable);
    DEBUGPRINT(4, "SuperBlock Info: root_cluster          : 0x%02x", g_Vmc_Image[unit].header.root_cluster);
    DEBUGPRINT(4, "SuperBlock Info: backup_block1         : 0x%02x", g_Vmc_Image[unit].header.backup_block1);
    DEBUGPRINT(4, "SuperBlock Info: backup_block2         : 0x%02x", g_Vmc_Image[unit].header.backup_block2);
    ...
    for (i = 0; g_Vmc_Image[unit].header.indir_fat_clusters[i] != 0; i++)
        DEBUGPRINT(4, "SuperBlock Info: indir_fat_clusters[%d] : 0x%02x", i, g_Vmc_Image[unit].header.indir_fat_clusters[i]);
    ...
    DEBUGPRINT(4, "SuperBlock Info: mc_type               : 0x%02x", g_Vmc_Image[unit].header.mc_type);
    DEBUGPRINT(4, "SuperBlock Info: mc_flag               : 0x%02x", g_Vmc_Image[unit].header.mc_flag);
